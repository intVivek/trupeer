...
export default function ZoomBlocksOverlay({ setOpenBlockEditor }) {
  const { zoomBlocks, setZoomBlocks, currentTime, width, height, dispatch } =
    useVideoContext();
  ...
  const handleZoomOverlayDrag = (event, block, i) => {
    ...
    const blockWidth = width / block.scaleFactor;
    const blockHeight = height / block.scaleFactor;
    ...
    const onMouseMove = (e) => {
      ...
      updatedX = Math.min(updatedX, width - blockWidth);
      updatedY = Math.min(updatedY, height - blockHeight);
          ...
          isActive && (
            <div
              ...
              style={{
                top: `${block.y}px`,
                left: `${block.x}px`,
                width: `${width / block.scaleFactor}px`,
                height: `${height / block.scaleFactor}px`,
